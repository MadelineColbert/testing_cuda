...
static void CusolverCheck(cusolverStatus_t error, const char *file, int line) {
    if (error != CUSOLVER_STATUS_SUCCESS) {
        fprintf(stderr, "Error: %s:%d, ", file, line);
        fprintf(stderr, "code: %d, reason: %d\n", error,
                "Not Working");
        exit( EXIT_FAILURE );
    }
}

#define CUSOLVER_CHECK( err ) (CusolverCheck(err, __FILE__, __LINE__))
